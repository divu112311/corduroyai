...
LOCAL_DOCS_FILE = "hts_rich_docs.json"

# ============ HELPER FUNCTIONS ============

def load_docs_from_gcs():
    """Load rich docs from GCS"""
    print("📥 Loading documents from GCS...", flush=True)
    ...
    print(f"   Loaded {len(docs)} documents from GCS", flush=True)
    return docs
...
def main():
    ...
    if os.path.exists(LOCAL_DOCS_FILE):
        ...
        all_docs = load_docs_from_gcs()
        # Save locally for faster access next time
        with open(LOCAL_DOCS_FILE, "w") as f:
    ...
    for batch_idx in range(0, len(remaining_docs), EMBEDDING_BATCH_SIZE):
        ...
        for i, doc in enumerate(batch_docs):
            ...
            vectors.append({
                ...
                'values': embeddings[i],
                'metadata': {
                    'description': entry.get('description', '') or components.get('description', ''),
                    'chapter': hts_code[:2],
                    'indent': str(entry.get('indent', '')),
                    'units': ', '.join(entry.get('units', [])) if isinstance(entry.get('units'), list) else str(entry.get('units', '')),
                    'general_rate': str(entry.get('general', '')),
                    'special_rate': str(entry.get('special', '')),
                }
            })
